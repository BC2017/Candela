...
├── game/             # games built on the engine (lightkeeper, emberwake)
...
**Post-roadmap: Emberwake — a procedural action dungeon-crawler.** The second game (`candela-emberwake`) raises the bar over Lightkeeper: seeded rooms-and-corridors dungeons (validated across 400 seeds by a pure-logic test), projectile combat where every ember bolt is a flying shadow-casting point light, three enemy behaviours (chasers with BFS repathing, ranged spitters, and a ring-bursting boss that summons adds at health thresholds), pickups, diegetic health (your flame *is* the health bar — the wisp's light gutters as you're hit), and a real ImGui HUD with title/death/victory screens rendered through a new engine game-UI pass (`RenderOptions.recordUI` now composites over the backbuffer outside the editor too). A `--autoplay` bot fights the whole game down to the brazier unattended — the headless end-to-end test, verified on software Vulkan. Games now live under `game/<name>/`, with shared grid logic (collision, BFS, flood fill) extracted to `game/common/Grid.h`.

**Post-roadmap: Lightkeeper — the first game on Candela.** `candela-lightkeeper`: you are a wisp of flame drifting through a night-shrouded shrine maze; touch each extinguished candle to relight it, and when the last one burns, dawn breaks over the maze in real time (the scene's sun, IBL, and exposure ramp from moonlight to morning — the win state is told entirely through light). WASD moves the wisp camera-relative, holding RMB orbits the follow camera, Escape quits; progress and the run timer live in the window title. Everything gameplay-visible is engine-driven: candles are `.glb` meshes whose `MeshRenderer` swaps from the unlit to the lit variant on ignition, flames and the wisp are emissive materials feeding bloom, and every flame is a flickering shadow-casting point light hung on a child entity *above* its mesh (a light at the centre of its own emitter is fully self-shadowed under RT shadows — found the hard way). The maze, collision (circle-vs-grid with corner-safe resolution), and BFS pathfinding live in a pure-logic header exercised by `lightkeeper-leveltest` (no GPU needed; proves every candle is reachable and collision never tunnels), and `--autoplay` is both an attract mode and a headless end-to-end test: the wisp pathfinds to every candle and wins the game unattended — verified on a GPU-less machine via Xvfb + lavapipe, which also drove a small `ShaderCompiler` portability fix (POSIX `slangc` discovery and quoting). Game assets (wisp, candle, wall, floor) were authored headlessly in Blender and live in `game/lightkeeper/content/`.

**Post-roadmap: Blender `.blend` import.** `.blend` files in the content directory are now first-class model assets: the registry converts them by running Blender headless with its bundled glTF exporter (cached as `.glb` under a `.candela-import/` folder next to the source, reconverted automatically when the `.blend` changes) and routes the result through the existing glTF pipeline — materials, textures, and the object hierarchy all come across. They appear in the content browser like any model, and **File > Import Blender Scene** opens one as a fresh scene — from the content directory (rescanned each time the menu opens) or from anywhere on disk via **Import .blend from Disk...** (native file dialog on Windows), which registers the file in place so it keeps hot-reloading when edited in Blender. Requires a Blender installation, auto-detected from `PATH` and standard install locations (override with the `CANDELA_BLENDER` environment variable).
