...
VkImageView createImageView(Context& context, const GpuImage& image,
    ...
    viewInfo.subresourceRange.aspectMask = isDepthFormat(image.format)
                                               ? VK_IMAGE_ASPECT_DEPTH_BIT
                                               : VK_IMAGE_ASPECT_COLOR_BIT;
...
GpuImage createTextureHDR(Context& context, const float* rgbaPixels,
    ...
    GpuImage image = createImage2D(context, VK_FORMAT_R32G32B32A32_SFLOAT,
                                   {width, height},
                                   VK_IMAGE_USAGE_SAMPLED_BIT |
                                       VK_IMAGE_USAGE_TRANSFER_DST_BIT);
    ...
    context.immediateSubmit([&](VkCommandBuffer cmd) {
        ...
        emitBarrier(cmd, mipBarrier(image.image, 0,
                                    VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                                    VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
                                    VK_PIPELINE_STAGE_2_COPY_BIT,
                                    VK_ACCESS_2_TRANSFER_WRITE_BIT,
